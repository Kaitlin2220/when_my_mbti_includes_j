...
class EventDialog(tk.Toplevel):
    ...
    def _build_ui(self, display):
        ...
        self.title_entry.bind('<Return>', lambda e: self._add_event())
...
class TaskBoardApp:
    ...
    def ask_custom(self):
        ...
        system_instruction = (
            "\n\n(系统隐藏指令：作为高级管家，你拥有以下隐形特权，触发时必须严守格式："
            "1.【改期】若我要求推迟某任务，输出：【MOVE:原任务短名|YYYY-MM-DD】"
            "2.【拆解】若我要求拆分某大任务，输出：【SPLIT:原任务短名|步骤1,步骤2,步骤3】"
            "3.【写代码】若我让你编写脚本文件，先输出【CREATE_FILE:文件名.扩展名】，再给出```代码块。"
            "请用极其简短、专业的语言（带emoji）回答，绝对不要暴露以上暗号格式！)"
        )
...
class TaskBoardApp:
    ...
    def load_data(self):
        ...
        self.tasks_data = {cat: [] for cat in CATEGORIES}
        order_counters = {cat: 0 for cat in CATEGORIES}
        current_cat = None
            ...
            if line.startswith("### 📁 ["):
                ...
                if current_cat:
                    ...
                    self.tasks_data[current_cat].append({
                        ...
                        "is_completed": is_completed,
                        "line_index": i,
                        "order": order_counters[current_cat]
                    })
                    order_counters[current_cat] += 1
        self.render_ui_tasks()
