...
class AchievementsDialog(tk.Toplevel):
    ...
    def _show_congrats_popup(self, path_name, node_name):
        """调 Claude 生成祝贺语，然后弹窗"""
        path_icon = "💼" if path_name == "找工作" else "📚"
        prompt = (
            f"用户刚刚解锁了成就！\n"
...
class TaskBoardApp:
    ...
    def load_daily_quote(self):
        ...
        def fetch_quote():
            prompt = "请用一句话鼓励一位正在处理经济学实证数据写论文、并备考雅思、找实习的研究生，该实习生容易焦虑，请你带一个emoji，直接输出这句话，绝对不要任何废话，字数控制在30字以内。"
